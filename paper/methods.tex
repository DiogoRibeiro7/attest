...
status---and that \emph{refusal} should be an admissible status. Section~2
states what the certificate attests. Section~3 recalls what split conformal
prediction guarantees and, more importantly, what it does not. Section~4 gives
...
infinite quantile. Section~5 turns to a separate and more mundane failure,
...
failure, which is the motivation for interval-based verdicts in Section~5.
...
which a conventional threshold fires when it should not.}
\label{tab:nulls}
\begin{tabular}{llrr}
\toprule
...
We close with the observation that motivated Section~5. Every defect we found
