...
The same principle applies to the leakage check, and applying it corrected a
defect that simulation had not exposed. Screening a feature for its ability to
reproduce the outcome by classification accuracy is a mistake whenever the
outcome is rare, because a majority-class rule already achieves the base rate.
On a portfolio of 67{,}856 motor policies \citep{insuranceData} with a 6.8\%
claim rate, all seven rating factors scored 0.9318 against a no-claim rate of
0.9319: the statistic was reporting the class balance and nothing else. The
effect replicates across four portfolios, where a pure-noise feature scores the
base rate to three decimal places at base rates from 0.53 to 0.99 while the
area under the curve stays at chance (Table~\ref{tab:proxy}). On one of them,
64{,}548 policies at a 1\% claim rate, accuracy scores 0.990 on noise and so
clears a 0.95 threshold: the screen refuses every model, precisely in the
rare-event settings the package is aimed at. Scoring by area under the curve,
or by mean per-class recall for multiclass outcomes, removes the dependence. On
the first portfolio the ordering then becomes actuarially sensible: exposure is
the strongest single predictor at 0.654 and gender the weakest at 0.503.

\begin{table}[ht]
\centering
\caption{Score given to a pure-noise feature on four insurance portfolios.
Accuracy tracks the base rate; area under the curve does not.}
\label{tab:proxy}
\begin{tabular}{lrrr}
\toprule
Portfolio & Base rate & Accuracy & Area under the curve \\
\midrule
\texttt{AutoBi}        & 0.527 & 0.527 & 0.504 \\
\texttt{dataCar}       & 0.932 & 0.932 & 0.506 \\
\texttt{SingaporeAuto} & 0.935 & 0.935 & 0.515 \\
\texttt{dataOhlsson}   & 0.990 & 0.990 & 0.512 \\
\bottomrule
\end{tabular}
\end{table}
